...
- [ ] T002 Delete the manual Northflank deployment workflow in `.github/workflows/deploy-northflank.yml` so CI has no deployment, verification, or rollback path.
...
- [ ] T003 [US1] Rewrite the CI-flow, image-versioning, and local-container sections in `docs/deployment.md` so publication to GHCR is the final repository-managed action and no external deployment result is implied.
- [ ] T004 [US1] Make the provider-neutral CI-boundary assertions pass in `tests/integration/deployment-artifacts.test.ts` for `docs/deployment.md` and `.github/workflows/publish-container.yml`.
...
- [ ] T005 [US2] Add the platform-neutral external-image handoff, registry-pull authorization boundary, and separately supplied runtime-configuration guidance in `docs/deployment.md`.
- [ ] T006 [US2] Replace the manual Northflank deployment instructions with immutable GHCR handoff and external-environment responsibilities in `README.md`.
- [ ] T007 [US2] Make the README and deployment-guide platform-neutral handoff assertions pass in `tests/integration/deployment-artifacts.test.ts`.
...
**Independent Test**: Search active README, operations documentation, workflows, and focused
delivery tests for the removed provider terminology; no active delivery instruction or runnable
workflow may remain, while historical `specs/007-container-deployment-pipeline/` records are
unchanged.

### Implementation for User Story 3

- [ ] T008 [P] [US3] Replace the provider deployment architecture description with the GHCR-only repository boundary and external-platform responsibility in `docs/architecture.md`.
- [ ] T009 [P] [US3] Update delivery-test coverage documentation to name GHCR publication and provider-neutral contract validation in `docs/testing.md`.
- [ ] T010 [US3] Finalize active-documentation and workflow-absence regression assertions in `tests/integration/deployment-artifacts.test.ts` without scanning historical feature specifications.
...
- [ ] T011 Run the quickstart review and reconcile any remaining wording or contract gaps in `specs/008-agnostic-container-delivery/quickstart.md`, `README.md`, `docs/deployment.md`, `docs/architecture.md`, `docs/testing.md`, and `tests/integration/deployment-artifacts.test.ts`.
- [ ] T012 Run `pnpm check` and, where Docker is available, `pnpm container:build` plus `pnpm container:smoke`; resolve feature-related failures in `.github/workflows/`, `tests/integration/deployment-artifacts.test.ts`, `README.md`, and `docs/`.
...
- **Foundational (Phase 2)**: Depends on T001; removal of the legacy deployment workflow blocks all
  story completion.
- **US1 (Phase 3)**: Depends on T002 because the guide must not describe an active removed workflow.
- **US2 (Phase 4)**: Depends on T003; it extends the same guide and README with the external
  handoff boundary.
- **US3 (Phase 5)**: Depends on T005–T007 so all primary operator guidance is stable before
  architecture, testing, and final regression wording are aligned.
- **Polish (Phase 6)**: Depends on all user-story work; T012 is the final quality gate.
...
- T008 and T009 can run in parallel because they modify different documentation files after US2.
- Documentation reviews may happen alongside contract-test execution, but edits to
  `tests/integration/deployment-artifacts.test.ts` (T001, T004, T007, T010) must be serialized.
- The Docker smoke commands in T012 may run alongside a completed `pnpm check` only in an environment
  where Docker is available.

## Parallel Example: User Story 3

```text
T008: docs/architecture.md
T009: docs/testing.md
```
...
1. Complete T001–T002 to make the GHCR-only automation boundary enforceable.
2. Complete T003–T004 so the deployment guide accurately describes that boundary.
...
- All 12 tasks use the required checkbox, sequential ID, story label where applicable, and exact
  target paths.
