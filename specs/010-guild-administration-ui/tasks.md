...
- [X] T077 [P] Update CI and publication assertions so frontend type/lint/test/build, Playwright/emulator E2E, one image, and the existing GHCR pipeline gate delivery in `.github/workflows/ci.yml`, `.github/workflows/publish-container.yml`, and `tests/integration/deployment-artifacts.test.ts`
