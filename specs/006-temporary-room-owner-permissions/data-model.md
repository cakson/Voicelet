...
| Owner permission state | `applied` or `failed` after the initial member-overwrite attempt. | Never report `applied` after failed/missing provider outcome; discarded with association. |
